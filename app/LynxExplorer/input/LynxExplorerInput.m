...
@interface LynxExplorerInput ()
@property(nonatomic, strong) UIColor *currentPlaceholderColor;
@end
...
- (void)applyPlaceholderAppearance {
  UIColor *placeholderColor = self.currentPlaceholderColor;
  if (placeholderColor == nil && self.view.textColor != nil) {
    placeholderColor = [self.view.textColor colorWithAlphaComponent:0.5];
  }
  if (placeholderColor == nil) {
    return;
  }

  NSString *placeholderText = self.view.placeholder ?: @"";
  if (@available(iOS 13.0, *)) {
    ...
    self.view.placeholder = placeholderText;
...
LYNX_PROP_SETTER("placeholder-color", setPlaceholderColor, NSString *) {
  self.currentPlaceholderColor = [UIHelper colorWithHexString:value];
  [self applyPlaceholderAppearance];
}
